...
def learnGame(keys): #keys is an array of values
    ...
    while running:
        ...
        # other means.
            ...
            if len(bombs) != 0:
                #getting the distances of the bombs from our turret
                distANDcoord = [(m.sqrt((bomb.x - t.x)**2 + (bomb.y - t.y)**2), (bomb.x, bomb.y)) for bomb in bombs]

                # Find the closest distance using the min() function
                dist = min(distANDcoord, key=lambda x: x[0])[0]
                #coordinates of the closest bomb
                coord = min(distANDcoord, key=lambda x: x[0])[1]

                #TargetXY < CLOSE value & target to the LEFT then ACTION
                if (int(dist) < int(keys[0]) & int(coord[0]) <= int(t.x)):
                    #switch
                    if keys[1][0] == "00":
                        t.moveMe(1)
                    elif keys[1][0] == "11":
                        ...
                        t.rotateMe(1)
                #TargetXY < CLOSE value & target to the RIGHT then ACTION
                elif (int(dist) < int(keys[0]) & int(coord[0]) > int(t.x)):
                    #switch
                    if keys[2][0] == "00":
                        t.moveMe(1)
                    elif keys[2][0] == "11":
                        ...
                        t.rotateMe(1)
                #TargetXY < NOT CLOSE value & target to the LEFT then ACTION
                elif (int(dist) > int(keys[0]) & int(coord[0]) <= int(t.x)):
                    #switch
                    if keys[3][0] == "00":
                        t.moveMe(1)
                        ...
                        t.rotateMe(1)
                #TargetXY < NOT CLOSE value & target to the RIGHT then ACTION
                elif (int(dist) > int(keys[0]) & int(coord[0]) > int(t.x)):
                    #switch
                    if keys[4][0] == "00":
                        t.moveMe(1)
...
class popMember:
    def __init__(self, keys, score=1000, win=False):
...
def breed(mom, dad):
    ...
    for i in range(1, 5):
        ...
        #get combination
            ...
            #coin flip which gene alleles are exchanged
...
def mutate(kid, prob, moves):
    newKid = []
    #0
    if random.random() < prob:
        newKid.append(random.randint(10,570))
    else:
        newKid.append(kid[0])
    #1  
    if random.random() < prob:
        newKid.append(random.choices(moves, k = 1))
    else:
        newKid.append(kid[1])
    #2    
    if random.random() < prob:
...
#OLD
def fitness(member):
    loopcount, turrethitCount, bulletsShot, bulletHits, bombsShotDown, targetsKilled, win = learnGame(member.keys)

    if (win):
        member.win = True
        #so if our member has won we will calculate it's score based on these stats
        member.score = bulletsShot*2 + turrethitCount*15 - bulletHits - bombsShotDown*6 - targetsKilled*2 - 100
        #if it didn't win we will keep it's score high
        #so the goal is to get score closest to 0
    else:
        member.score = bulletsShot*2 + turrethitCount*15 - bulletHits - bombsShotDown*6 - targetsKilled*2  + 200

nGnrts = 10
#00 - up, 11 - down, 01 - right, 10 - left
moves = ["00", "01", "10", "11"]
for j in range(nGnrts):
    if j == 0:
        for i in range (10):
            keys = []  
            keys.append(random.randint(10,570))
            keys.append(random.choices(moves, k = 1))
            keys.append(random.choices(moves, k = 1))
            keys.append(random.choices(moves, k = 1))
            keys.append(random.choices(moves, k = 1))
            
            pop.append(popMember(keys))
        popSize = len(pop)
    ...
    for k in range(d, popSize):
        ...
        kid = breed(pop[mom].keys, pop[dad].keys)
        kid = mutate(kid, 0.1, moves)
